...
import { UserSessionInformation, UserSessionsInformations } from 'src/app/services/session.fixtures';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { sessionApiFixture } from './session-api.fixture';
...
describe('SessionsService', () => {
  ...
  it('should return all sessions', () => {
    service.all().subscribe(sessions => {
      ...
      expect(sessions).toEqual(UserSessionsInformations)
    });

    const req = httpMock.expectOne(`api/session`);
    expect(req.request.method).toBe('GET');
    req.flush(UserSessionsInformations);
  });
  ...
  describe('detail', () => {
    it('should return session detail', () => {
      service.detail('1').subscribe(session => {
        ...
        expect(session).toEqual(UserSessionInformation);
      });

      const req = httpMock.expectOne(`api/session/1`);
      expect(req.request.method).toBe('GET');
      req.flush(UserSessionInformation)
    });
  ...
  it('should create session', () => {
    service.create(sessionApiFixture).subscribe(session => {
      expect(session).toBeTruthy();
      expect(session).toEqual(sessionApiFixture);
    });

    const req = httpMock.expectOne(`api/session`);
    expect(req.request.method).toBe('POST');
    req.flush(sessionApiFixture);
  });

  describe('updated', () => {
    it('should update session', () => {
      service.update('1', sessionApiFixture).subscribe(session => {
        expect(session).toBeTruthy();
        expect(session).toEqual(sessionApiFixture);
      });
    ...
    it('should not update session because invalid id', () => {
      service.update('invalid-id', sessionApiFixture).subscribe(session => {
        expect(session).toBeFalsy();
        expect(session).toEqual(null);
      });
